...
int omain(int argc, char* argv[])
{
	...
	WAITABLE_HANDLE infraredWaitable;
	hResult = pInfraredReader->SubscribeFrameArrived(&infraredWaitable);
	if (FAILED(hResult)){
		std::cerr << "Error : InfraredFrameReader::SubscribeFrameArrived()" << std::endl;
		std::cin >> hResult;
		return -1;
	}
	...
	while (1){
		// Frame
		HANDLE hEvents[] = { reinterpret_cast<HANDLE>(infraredWaitable) };
		WaitForMultipleObjects(ARRAYSIZE(hEvents), hEvents, true, INFINITE);

		// Arrived Data
		IInfraredFrameArrivedEventArgs* pInfraredArgs = nullptr;
		hResult = pInfraredReader->GetFrameArrivedEventData(infraredWaitable, &pInfraredArgs);
		if (SUCCEEDED(hResult)){
			IInfraredFrameReference * pInfraredReference = nullptr;
			hResult = pInfraredArgs->get_FrameReference(&pInfraredReference);
			if (SUCCEEDED(hResult)){
